...
public class ChartReportServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {	
		try { 
			...
			System.out.print("ChartReportServlet: Stats asked by "+ ((UserBean)session.getAttribute("currentSessionUser")).getFullname());
			List<ReportStatsBean> stats = ReportStatsDAO.listStatsByMounth();
			session.setAttribute("listStats",stats); 
			response.sendRedirect("chartreports.jsp");
		} catch (Throwable e) { 
				e.printStackTrace();
		}
	}
